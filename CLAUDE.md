...
| `src/formatter/python.rs` | Python formatter. **Stub** — parses with tree-sitter, returns source unchanged (same bootstrapping stage `c_cpp.rs` started from). No single reference tool exists (PEP8 is a style guide, not a formatter); the structural target is `ruff format` (Black-compatible), and `flake8` is used as a lint gate to confirm formatted output is PEP8-clean. `Config.python.max_width` defaults to 79 (PEP8/flake8); `--preset black` sets 88 to match `ruff format`. `tests/python_corpus_test.rs` diffs against static `.expected` files (currently == input, since fixtures were hand-picked to already be ruff-format/flake8-clean) plus an `#[ignore]`d test that shells out to the real `ruff`/`flake8` binaries to verify that property — run it explicitly with `cargo test --test python_corpus_test -- --ignored` when adding fixtures or real emission logic. |
